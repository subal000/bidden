...
var (
	...
	discStartBidding     = []byte{0x32, 0x04, 0x34, 0xdb, 0x72, 0x19, 0x46, 0xb9}
...
// ixStartBidding opens the auction and stamps its deadline from the ER clock.
// It must run in the ER, not on L1: the two use different slot domains, so a
// deadline written on L1 is already in the ER's past.
func ixStartBidding(job, requester solana.PublicKey, durationSlots uint64) solana.Instruction {
	data := make([]byte, 0, 16)
	data = append(data, discStartBidding...)
	d := make([]byte, 8)
	binary.LittleEndian.PutUint64(d, durationSlots)
	data = append(data, d...)
	return solana.NewInstruction(programID, solana.AccountMetaSlice{
		solana.NewAccountMeta(job, true, false),
		solana.NewAccountMeta(requester, false, true),
	}, data)
}
