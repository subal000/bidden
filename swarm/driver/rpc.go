...
// Balance reads at "confirmed". Without an explicit commitment getBalance
// defaults to finalized, which lags a just-landed transaction by many seconds
// and made settle report an untouched escrow and an unpaid winner.
...
// MultipleAccounts fetches many accounts in one request. Polling seven accounts
// individually once a second is 7 req/s against a public RPC that caps a single
// method near 40 per 10s, so the reads start failing and a failed read is
// indistinguishable from an account that has not undelegated.
func (c *Client) MultipleAccounts(ctx context.Context, pks []solana.PublicKey) ([]*AccountInfo, error) {
	keys := make([]string, len(pks))
	for i, p := range pks {
		keys[i] = p.String()
	}
	res, err := c.call(ctx, "getMultipleAccounts", keys,
	...
	out := make([]*AccountInfo, len(v.Value))
	for i, a := range v.Value {
		...
		out[i] = &AccountInfo{Owner: solana.MustPublicKeyFromBase58(a.Owner), Data: data}
	}
	return out, nil
}
